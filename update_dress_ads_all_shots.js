...
async function updateDressAdsShots() {
  ...
  if (shots.length === 0) {
    console.log("No shots found for Dress > Ads > Model Photo");
    return;
  }
  ...
  for (const shot of shots) {
    ...
    console.log(`Updated Shot ${shot.shotNumber} (Dress Ads) - ID: ${shot.id} - Presentation: ${shot.presentation}`);
  }
}

updateDressAdsShots().catch(console.error).finally(() => prisma.$disconnect());
