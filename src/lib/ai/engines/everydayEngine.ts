...
export async function generateEverydayImages({
    qty,
    subcat,
    publicUrls,
    userClarification,
...
REGOLE ASSOLUTE E INVIOLABILI PER PRESERVARE L'INDUMENTO:
1. PRESERVAZIONE STRUTTURALE AL 100%: E' SEVERAMENTE VIETATO modificare o immaginare diversamente la scollatura, le cuciture, i dettagli, la lunghezza del capo o delle maniche, cinture e bottoni. L'indumento deve essere perfettamente identico.
2. PRESERVAZIONE MATERIALE AL 100%: Il colore ESATTO, il tipo di tessuto (seta, lana, cotone pesante, denim, pelle, ecc) e la texturizzazione visiva devono essere identici all'originale. Non aggiungere stampe, o increspature che non esistono nella foto.
3. ADATTAMENTO CORPOREO E VOLTI: Se lo stile richiede una modella/o, la persona DEVE avere un VISO e OCCHI A FUOCO, NITIDI.
4. RIMOZIONE CARTELLINI E ETICHETTE INTERNE: Se l'immagine originale contiene un cartellino del negozio (price tag) o un'ETICHETTA INTERNA DEL BRAND visibile, ESSI DEVONO SPARIRE. Il colletto deve essere pulito e naturale.
5. FOCUS SUL CAPO ORIGINALE (NO EXTRA LAYERS): Se l'immagine in input ritrae una t-shirt, camicia, giacca o gonna, E' ASSOLUTAMENTE VIETATO coprirlo parzialmente con cappotti, giacche, felpe, o scialli non presenti nella foto originale. L'indumento inserito dal cliente deve essere esaltato e mostrato per intero senza coperture spurie.
6. VARIETA' (Batch): Genera pose naturali e diverse tra loro ispirate al dataset fotografico dello Stile.
7. NO ATTREZZATURA: È ASSOLUTAMENTE VIETATO includere luci da set, softbox, cavalletti o macchine fotografiche. L'ambiente deve essere puro e senza backstage visibile.
8. FRONT PRINT EXCLUSIVITY: Se non hai ricevuto un'immagine specifica per il retro (BACK) e questo capo ha una stampa sul fronte, IL RETRO DEVE ESSERE COMPLETAMENTE BIANCO/VUOTO (tinta unita). È ASSOLUTAMENTE VIETATO copiare o far comparire la stampa frontale sul retro nelle foto scattate da dietro.
${userClarification === 'UGC_MAN' ? `9. CLARIFICATION FROM THE USER: The user has explicitly requested a MALE model for this shot. YOU MUST STRICTLY USE A HANDSOME 20-25 YEAR OLD BOY. ABSOLUTELY NO FEMALE MODELS.` : (userClarification !== 'X' ? `9. CLARIFICATION FROM THE USER: The user was asked a question about the garment and explicitly responded with: "${userClarification}". YOU MUST STRICTLY RESPECT THIS INFORMATION AND BUILD THE IMAGE ACCORDINGLY.` : '')}
${isOutfit ? `10. [STRICT APPAREL OUTFIT COORDINATION: The user has uploaded MULTIPLE separate garments (e.g., shirt, pants, jacket, bag). You MUST dress the model wearing ALL of these items together perfectly in the same image. Ensure precise, professional styling without merging the items. Do NOT mix up the items.]` : ''}
${taxonomySubcat?.toLowerCase().includes('model photo') ? `11. MODEL REALISM (NO PLASTIC): I modelli DEVONO essere esteticamente bellissimi, con lineamenti eleganti. TUTTAVIA, DEVI GARANTIRE UN REALISMO ESTREMO FOTOGRAFICO. La pelle NON deve sembrare di plastica, lisciata, CGI o videogioco. Mostra la texture naturale della pelle e un'illuminazione fotografica reale.` : ''}
${(taxonomyCat?.toLowerCase().includes('everyday') && taxonomyMode?.toLowerCase().includes('detail')) ? `12. CRITICAL GARMENT DETAIL PRESERVATION (DOUBLE ANALYSIS): Esegui una DOPPIA ANALISI e procedi con ESTREMA CAUTELA sui piccoli dettagli strutturali del capo d'abbigliamento. FAI ATTENZIONE a nastri o grucce: sono elementi ESTERNI e INDIPENDENTI, NON fanno parte della struttura del capo.` : ''}`;
