...
export default function DashboardWizard({ snippets, isAdmin, activeCategories = [], activeBusinessModes = [], activeSubcategories = [] }: { snippets: Snippet[], isAdmin?: boolean, activeCategories?: any[], activeBusinessModes?: any[], activeSubcategories?: any[] }) {
  ...
  const handleGenerate = async () => {
    ...
    if (isUGC) {
       const ugcRules = "[UGC REALISM MODE (HARD): This must NOT look like a professional photoshoot, BUT the model must be exceptionally beautiful and highly attractive (Instagram Influencer aesthetic). Simulate a real iPhone photo. Rules: imperfect framing, slight motion blur allowed, uneven natural lighting, casual pose (not posed, not fashion runway), handheld feeling, slightly tilted horizon allowed, depth of field must be flat (like smartphone). Camera simulation: iPhone camera, no cinematic blur, no professional lens look. IMPORTANT: This must feel like a gorgeous influencer taking a casual lifestyle photo at home for social media, NOT a fashion brand campaign. Do not add accessories unless explicitly requested.] ";
