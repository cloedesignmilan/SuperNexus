  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
        <a href="?" style={{ padding: '0.5rem 1rem', borderRadius: '20px', background: !filterType ? '#00d2ff' : 'rgba(255,255,255,0.05)', color: !filterType ? '#0f172a' : 'var(--color-text-muted)', fontSize: '0.8rem', textDecoration: 'none', fontWeight: !filterType ? 700 : 400 }}>Tutti</a>
        {snippetTypes.map(t => (
          <a key={t} href={`?type=${t}`} style={{ padding: '0.5rem 1rem', borderRadius: '20px', background: filterType === t ? '#00d2ff' : 'rgba(255,255,255,0.05)', color: filterType === t ? '#0f172a' : 'var(--color-text-muted)', fontSize: '0.8rem', textDecoration: 'none', fontWeight: filterType === t ? 700 : 400 }}>
            {t}
          </a>
        ))}
      </div>
        ...
        <div>
          <div className="glass-card" style={{ position: 'sticky', top: '2rem', padding: '1.5rem', background: '#1c1c1e', borderRadius: '24px', border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <form action={createSnippet as any} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <button type="submit" style={{ marginTop: '1rem', background: '#00d2ff', color: '#1c1c1e', padding: '0.75rem', borderRadius: '12px', border: 'none', fontWeight: 700, cursor: 'pointer' }}>
                Crea Snippet
              </button>
            </form>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {snippets.length === 0 ? (
            ...
            snippets.map((snip: any) => (
              <div key={snip.id} className="glass-card" style={{ padding: '1.5rem', display: 'flex', gap: '1rem', alignItems: 'flex-start', borderLeft: `4px solid ${snip.is_active ? '#00d2ff' : '#475569'}`, background: '#1c1c1e', borderRadius: '24px', borderTop: '1px solid rgba(255,255,255,0.05)', borderRight: '1px solid rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
