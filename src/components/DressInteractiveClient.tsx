      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
          {activeModes.map((mode, i) => (
              <div 
                  ...
                  style={{ 
                      background: validSelectedMode === mode.name ? 'rgba(204, 255, 0, 0.1)' : 'linear-gradient(180deg, #151515 0%, #0a0a0a 100%)', 
                      border: validSelectedMode === mode.name ? '1px solid #ccff00' : '1px solid rgba(255,255,255,0.1)', 
                      borderRadius: '16px', 
                      padding: '2rem 1.5rem', 
                      textAlign: 'center', 
                      ...
                      boxShadow: validSelectedMode === mode.name ? '0 10px 20px rgba(204, 255, 0, 0.1)' : 'none'
                  }} 
                  className={validSelectedMode !== mode.name ? "hover-scale" : ""}
              >
                  <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '60px', height: '60px', borderRadius: '50%', background: validSelectedMode === mode.name ? 'rgba(204, 255, 0, 0.2)' : 'rgba(255,255,255,0.05)', marginBottom: '1.5rem', transition: 'all 0.3s ease' }}>
                      <mode.icon size={28} color={validSelectedMode === mode.name ? "#ccff00" : "#fff"} />
                  </div>
                  <h4 style={{ color: validSelectedMode === mode.name ? '#ccff00' : '#fff', fontSize: '1.1rem', fontWeight: '700', marginBottom: '0.5rem', transition: 'color 0.3s ease' }}>{mode.name}</h4>
