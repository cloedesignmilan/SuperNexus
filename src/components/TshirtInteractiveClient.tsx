      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
          {modes.map((mode, i) => (
              <div 
                  ...
                  style={{ 
                      background: selectedMode === mode.name ? 'rgba(255, 84, 112, 0.1)' : 'linear-gradient(180deg, #151515 0%, #0a0a0a 100%)', 
                      border: selectedMode === mode.name ? '1px solid #ff5470' : '1px solid rgba(255,255,255,0.1)', 
                      borderRadius: '16px', 
                      padding: '2rem 1.5rem', 
                      textAlign: 'center', 
                      ...
                      boxShadow: selectedMode === mode.name ? '0 10px 20px rgba(255, 84, 112, 0.1)' : 'none'
                  }} 
                  className={selectedMode !== mode.name ? "hover-scale" : ""}
              >
                  <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '60px', height: '60px', borderRadius: '50%', background: selectedMode === mode.name ? 'rgba(255, 84, 112, 0.2)' : 'rgba(255,255,255,0.05)', marginBottom: '1.5rem', transition: 'all 0.3s ease' }}>
                      <mode.icon size={28} color={selectedMode === mode.name ? "#ff5470" : "#fff"} />
                  </div>
                  <h4 style={{ color: selectedMode === mode.name ? '#ff5470' : '#fff', fontSize: '1.1rem', fontWeight: '700', marginBottom: '0.5rem', transition: 'color 0.3s ease' }}>{mode.name}</h4>
